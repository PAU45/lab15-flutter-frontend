import 'dart:convert';
import 'package:http/http.dart' as http;
import 'mesa.dart';

class MesaService {
  static const _baseUrl = 'http://localhost:3000/api/mesas';

  Future<List<Mesa>> fetchMesas() async {
    final res = await http.get(Uri.parse(_baseUrl), headers: {
      'Accept': 'application/json',
      'User-Agent': 'Mozilla/5.0 (Android 14; Mobile) Flutter',
    });
    if (res.statusCode == 200) {
      final data = jsonDecode(res.body) as List;
      return data.map((e) => Mesa.fromJson(e)).toList();
    }
    throw Exception('Error ${res.statusCode}');
  }

  Future<Mesa> createMesa(Mesa mesa) async {
    final res = await http.post(
      Uri.parse(_baseUrl),
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
        'User-Agent': 'Mozilla/5.0 (Android 14; Mobile) Flutter',
      },
      body: jsonEncode(mesa.toJson()),
    );
    if (res.statusCode == 201 || res.statusCode == 200) {
      return Mesa.fromJson(jsonDecode(res.body));
    }
    throw Exception('Error ${res.statusCode}');
  }

  Future<Mesa> updateMesa(Mesa mesa) async {
    if (mesa.id == null) throw Exception('ID requerido para actualizar');
    final res = await http.put(
      Uri.parse('$_baseUrl/${mesa.id}'),
      headers: {
        'Content-Type': 'application/json',
        'Accept': 'application/json',
        'User-Agent': 'Mozilla/5.0 (Android 14; Mobile) Flutter',
      },
      body: jsonEncode(mesa.toJson()),
    );
    if (res.statusCode == 200) {
      return Mesa.fromJson(jsonDecode(res.body));
    }
    throw Exception('Error ${res.statusCode}');
  }

  Future<void> deleteMesa(int id) async {
    final res = await http.delete(
      Uri.parse('$_baseUrl/$id'),
      headers: {
        'Accept': 'application/json',
        'User-Agent': 'Mozilla/5.0 (Android 14; Mobile) Flutter',
      },
    );
    if (res.statusCode != 200 && res.statusCode != 204) {
      throw Exception('Error ${res.statusCode}');
    }
  }
}
